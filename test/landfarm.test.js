...
// HEAD's (609d796) pristine parse of the fixture, regenerated by running that
// commit's landfarm.js against the same fixture. Guards that today's parser
// still emits byte-identical output for an untouched capture.
const PRISTINE_LISTINGS = JSON.parse(fs.readFileSync(
  path.join(__dirname, 'fixtures', 'landfarm-pristine-listings.json'),
  'utf8',
));

/**
 * Locate the exact `id="placard-image"` occurrence belonging to the card
 * whose href contains `slug`, so a single card's image id can be renamed
 * without touching any other card's. Each card owns exactly one
 * placard-image between its own placard-container start and the next one (or
 * the end of the document) — mirrored from how the existing single-container
 * test locates its target by raw string position.
 */
function placardImagePositionForSlug(html, slug) {
  const containerPositions = [...html.matchAll(/id="placard-container"/g)].map(m => m.index);
  const imagePositions = [...html.matchAll(/id="placard-image"/g)].map(m => m.index);
  const escapedSlug = slug.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const hrefPositions = [...html.matchAll(new RegExp(escapedSlug, 'g'))]
    .map(m => m.index)
    .filter(p => p >= containerPositions[0]);
  assert.ok(hrefPositions.length > 0, `${slug} must appear inside a placard card`);

  let containerIndex = -1;
  for (let i = 0; i < containerPositions.length; i++) {
    if (containerPositions[i] <= hrefPositions[0]) containerIndex = i;
    else break;
  }
  assert.ok(containerIndex >= 0, `${slug} must fall within a placard-container`);

  const cardStart = containerPositions[containerIndex];
  const cardEnd = containerIndex + 1 < containerPositions.length ? containerPositions[containerIndex + 1] : html.length;
  const imagesInCard = imagePositions.filter(p => p >= cardStart && p < cardEnd);
  assert.equal(imagesInCard.length, 1, `${slug}'s card must have exactly one placard-image`);
  return imagesInCard[0];
}

/** Rename the single `id="placard-image"` occurrence starting at `position`. */
function renamePlacardImageAt(html, position) {
  return `${html.slice(0, position)}id="listing-image"${html.slice(position + 'id="placard-image"'.length)}`;
}
...
test('pristine fixture output is byte-identical to HEAD (609d796)', () => {
  // Regression guard for the placard-image coverage fix: it must change
  // nothing about how an untouched capture parses. Baseline regenerated by
  // running HEAD's landfarm.js against this same fixture (see
  // test/fixtures/landfarm-pristine-listings.json).
  const { listings } = parseFixture();
  assert.equal(PRISTINE_LISTINGS.length, 18);
  assert.deepEqual(listings, PRISTINE_LISTINGS, 'every field of every row must match HEAD exactly');
});
...
test('renaming ONLY placard-image (container intact) raises drift and leaks NO Pending listings', async () => {
  // The finding this guards: placard-container and placard-image are
  // independent ids — renaming one does not rename the other. Before the
  // fix, container coverage stayed 25/25 (no drift from that check), the
  // per-card status read resolved against nothing, status='' failed
  // UNAVAILABLE_STATUS_RE, and all 7 Pending cards leaked in as buyable New
  // Leads (25 emitted, sourceIssues empty). This is the second, image-only
  // coverage check that closes that leak.
  const renamed = FIXTURE.replace(/id="placard-image"/g, 'id="listing-image"');
  assert.ok(renamed.includes('id="placard-container"'), 'the container id must stay untouched');
  assert.ok(!renamed.includes('id="placard-image"'), 'every placard-image must actually be renamed');

  const direct = new LandAndFarmParser();
  assert.deepEqual(direct.parseSearchPage(renamed, 'Wayne', 'KY'), [], 'no unvetted rows are emitted');
  assert.equal(direct._lastCardCount, 0, 'image-only loss must also drop the count to 0, the signal base-parser watches');
...
test('one missing placard-image on a non-Pending card excludes only that card as unknowable', () => {
  const position = placardImagePositionForSlug(FIXTURE, 'olde-tennessee-trail-140-41217070');
  const oneRenamed = renamePlacardImageAt(FIXTURE, position);
  assert.notEqual(oneRenamed, FIXTURE, 'the fixture must actually be modified');
  assert.equal([...oneRenamed.matchAll(/id="placard-image"/g)].length, 24,
    'exactly one placard-image must be gone');
  ...
  assert.equal(parsed.length, 17, 'the image-less card is excluded (unknowable status), the other 17 non-Pending cards are unaffected');
  assert.ok(!parsed.some(l => l.url.includes('olde-tennessee-trail-140-41217070')),
    'a card with no resolvable placard-image has unknowable status, so it is skipped rather than guessed');

  const prunedPristine = PRISTINE_LISTINGS.filter(l => !l.url.includes('olde-tennessee-trail-140-41217070'));
  assert.deepEqual(parsed, prunedPristine, 'every other row is byte-identical to the pristine parse');
});

test('one missing placard-image on an already-Pending card changes nothing observable', () => {
  const pendingSlug = PENDING_SLUGS[0];
  const position = placardImagePositionForSlug(FIXTURE, pendingSlug);
  const oneRenamed = renamePlacardImageAt(FIXTURE, position);
  assert.notEqual(oneRenamed, FIXTURE, 'the fixture must actually be modified');
  ...
  const parsed = direct.parseSearchPage(oneRenamed, 'Wayne', 'KY');
  assert.equal(direct._lastCardCount, 25, 'all JSON-LD listings still count as cards');
  // The Pending card was already excluded on pristine input (18 emitted, not
  // 25) — losing its placard-image just swaps WHY it is excluded (unknowable
  // status instead of a Pending badge), so the emitted count stays 18, not
  // 18-1.
  assert.equal(parsed.length, 18, 'an already-excluded Pending card losing its image changes nothing observable');
  assert.deepEqual(parsed, PRISTINE_LISTINGS, 'output is byte-identical to the pristine parse');
});
