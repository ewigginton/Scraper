...
//
// Tolerance: isBlockedHtml strips any whitespace (a trailing space, a
// newline) that sits directly before `</title>` before matching, so
// '<title>LandWatch / 400 </title>' and a newline-before-close variant still
// hit the '/ 400</title>' marker. Nothing else about the markers is
// whitespace-tolerant — this narrow fix is scoped to the one junction real
// captures are known to vary on.
const BLOCK_MARKERS = [
...
function isBlockedHtml(html) {
  ...
  // Collapse whitespace immediately before </title> only, so a captured
  // '<title>... / 400 </title>' (trailing space) or a newline before the
  // close still lines up with the tight '/ 400</title>' marker below.
  const sample = html.slice(0, 20000).toLowerCase().replace(/\s+<\/title>/g, '</title>');
