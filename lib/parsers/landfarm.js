 ...
 * renamed without the other — so BOTH are coverage-checked, but against the
 * SAME denominator (the JSON-LD item count) and the SAME MIN_PLACARD_COVERAGE
 * floor, so the two losses cannot compound past that single floor. Container
 * loss makes county AND status unknowable; image-only loss makes status alone
 * unknowable (county still resolves off the container's own address line),
 * and status unknowable is exactly as dangerous as status missing entirely —
 * an unvetted Pending card is a buyable New Lead. Either coverage collapse
 * reports the page through the REAL drift mechanism in base-parser.scrapeAll
 * (zero listings + `_lastCardCount = 0` on page 1) rather than emitting
 * unvetted rows. Below that page-level threshold, a per-card image miss is
 * handled the same way a per-card container miss always was: the card is
 * counted (it participates in `_lastCardCount`) but never emitted, because
 * its status cannot be vetted. See MIN_PLACARD_COVERAGE for the partial-loss
 * threshold.
 *
...
class LandAndFarmParser extends BaseParser {
  ...
  parseSearchPage(html, county, state) {
    ...
    // status — does not. That is coverage-checked exactly like the container
    // itself, because a page-wide image loss is just as unknowable a status
    // signal as a page-wide container loss — and against the SAME `items.length`
    // denominator as the container check above (NOT `matchedItems.length`), so
    // a partial container loss and a partial image loss share one 20%-of-page
    // budget instead of each independently tolerating 20% and silently
    // compounding to a much larger combined loss.
    const imageResolvedCount = matchedItems.reduce(
      (count, item) => count + (placards.get(item.path).imageFound ? 1 : 0), 0);
    if (imageResolvedCount < items.length * MIN_PLACARD_COVERAGE) {
      this._lastCardCount = 0;
      console.warn(`[${this.name}] Placard image lost: ${imageResolvedCount}/${items.length} JSON-LD listings had a resolvable placard-image — reporting as markup drift`);
      return [];
    }
...
// the page is trusted — checked TWICE, against the SAME denominator
// (`items.length`, the JSON-LD count), because `placard-container` and
// `placard-image` are two separate ids that can be renamed one at a time:
// once for JSON-LD listings vs `placard-container` (county AND the
// status-lookup scope), and again for matched listings vs `placard-image`
// (sale status alone) — but measured as a fraction of ALL items, not just
// the survivors of the first check, so the two checks share one budget
// instead of each independently tolerating loss and compounding. Chosen so
// that ordinary partial loss (a lazily rendered tile, one malformed card)
// does NOT nuke a real page — on the 25-card capture, total structural loss
// across BOTH ids combined is tolerated up to floor(0.2 * items) (5 on this
// capture) — while a bigger combined loss, including a structural loss (an
// id renamed page-wide takes its coverage to 0), is reported as drift. Items
// that still lack a container or an image after both checks pass are counted
// but not emitted: their sale status is unknown (unmatched container) or
// unknowable (container found, image missing), and a Pending row is worse
// than a missed one either way.
