...
const { ACREAGE_NUMBER, parseAcreageNumber, extractStructuredTextAcres } = require('../acreage');
...
class BaseParser {
   ...
   * detectable after parsing). An HTTP 400 is NOT retried through the browser:
   * it rejects the URL shape, not the client, so a browser attempt burns ~30s
   * on a guaranteed identical failure (see browser-fetch isUnsupportedUrlStatus).
   */
