...
// Unsupported-URL (HTTP 400) circuit breaker. A handful of these is the
// expected case they're excluded from the bot-wall counter for — one bad
// county slug or an unconfirmed filter segment, where the rest of the run is
// fine. A SITEWIDE URL-scheme break looks different: every county fails, and
// with series exhaustion capping it at ~2 requests per county, 12 means the
// run stops after ~6 counties instead of walking all 189 (each a full headed
// Chrome navigation on browser-render sources). The repo has paid for the
// unbounded version before — commit 6875f3b logged 2,525 failed LandWatch
// fetches over ~6h.
const UNSUPPORTED_URL_ABORT_THRESHOLD = 12;

// Unrendered-SPA-shell detection (see looksLikeUnrenderedShell). A real page
// carries nav, footer and body copy well past this even when its listing cards
// have been redesigned, so the threshold separates "no content at all" from
// "content we no longer recognize".
const MAX_UNRENDERED_SHELL_TEXT_CHARS = 800;
...
// Browser renders are expensive, so a site that ISN'T merely unrendered stops
// costing after a few attempts — but one that recovers is allowed to keep
// going, because at that point every retry is returning real listings.
const MAX_UNRENDERED_SHELL_RETRIES = 5;
...
class BaseParser {
  constructor(name) {
    this.name = name;
    this.stats = { checked: 0, parsed: 0, errors: 0, blockedPages: 0, errorPages: 0, unsupportedUrlPages: 0, driftPages: 0, snapshotsSaved: 0, browserFetches: 0, abortedByBotWall: false, abortedAt: null, earlyStoppedSeries: 0, shellRetries: 0, shellRecoveries: 0 };
    this.sourceIssues = [];
   ...
   * refused outright with a bot-wall status (403/429/503). Challenge pages
   * served with HTTP 200 are handled separately in scrapeAll (they're only
   * detectable after parsing).
   *
   * HTTP 400 is retried through the browser by DEFAULT, because some bot
   * managers answer a bad TLS/HTTP2 fingerprint with 400 rather than 403 — the
   * wall commit 6875f3b was written to get past. Callers that build their own
   * URLs (scrapeAll's search pages) pass browserRetryOn400:false, since there a
   * 400 means the URL shape is unsupported and the retry is pure waste.
   */
  async fetchPageSmart(url, { browserRetryOn400 = true } = {}) {
    try {
      return await this.fetchPage(url);
    } catch (err) {
      const retryable = browserFetch.isBotWallStatus(err.status)
        || (browserRetryOn400 && browserFetch.isUnsupportedUrlStatus(err.status));
      if (!retryable || !browserFetch.isEnabled()) throw err;
      try {
  ...
  /**
   * Extract a listing's TRUE acreage from its detail page, for the
   * post-enrichment cross-check in lib/scraper.js (applyDetailToListing) that
   * fixes the "N-acre lake" bug: a search card's acreage guess can be
   * corrupted by a description phrase like "on the gorgeous 1,000 acre Lake
   * Halford" (a real 0.94ac LandWatch listing that got recorded as 1000ac).
   * Sources are tried in DESCENDING trustworthiness, each naming the
   * LISTING's own acreage rather than a number that merely appears in nearby
   * prose:
   *   1. Structured data — JSON-LD (schema.org acreage/lotSize/landArea
   *      fields) and price-adjacent text patterns ("$150,000 • 0.94 acres",
   *      "View 0.94 acres priced at ...") — see lib/acreage.js.
   *   2. The page's own title/heading ("160 Acre Farm").
   *   3. Free-text description, LAST resort and only when nothing above
   *      matched — this is exactly where the lake/park/development acreage
   *      bug lived, so it must never be checked first.
   * All three route acreage-number parsing through the shared ACREAGE_NUMBER
   * fragment (lib/acreage.js), so a leading-decimal figure ('.499 Acres +/-')
   * is never truncated into a huge bogus number here either.
   *
   * MUST NOT throw on weird/garbage HTML — enrichment is a non-fatal
   * best-effort step, so any failure returns null.
   */
  extractDetailAcres(html) {
    try {
      if (!html || typeof html !== 'string') return null;
      const guarded = html.length > MAX_DETAIL_PARSE_BYTES || countTags(html) > MAX_DETAIL_PARSE_TAGS;
      const scanHtml = guarded ? html.slice(0, 200000) : html;

      const structured = acresFromJsonLd(scanHtml);
      if (structured !== null) return structured;
      const structuredText = extractStructuredTextAcres(scanHtml);
      if (structuredText !== null) return structuredText;

      const heading = headingText(scanHtml);
      if (heading) {
        const fromHeading = this.extractAcres(heading);
        if (fromHeading !== null) return fromHeading;
      }

      const description = this.extractDetailDescription(html);
      return this.extractAcres(description);
    } catch (err) {
      // Defensive: enrichment must never break a scrape. Swallow and yield null.
      return null;
    }
  ...
  /**
   * Acreage from the detail page that is UNAMBIGUOUSLY this listing's own —
   * the only signal strong enough to OVERWRITE an acreage the search card
   * already supplied.
   *
   * extractDetailAcres above scans the whole page and returns the first match
   * in document order, which is right for filling a MISSING acreage (there is
   * nothing to destroy) but wrong for overriding: detail pages carry "similar
   * properties nearby" rails whose cards use identical markup, so a neighbour's
   * figure can win. A verified reproduction rewrote a correct 200ac listing to
   * a neighbour's 1.5ac, after which the 40-acre floor silently dropped a real
   * lead — the same class of corruption this extractor exists to prevent.
   *
   * Unambiguous means price-anchored: the acreage is rendered alongside a
   * dollar figure matching the listing's own price ("$574,000 • 80 acres"), so
   * it cannot be a different property's card. Returns null when the listing has
   * no price or nothing matches; the caller must then report the drift rather
   * than act on it.
   */
  extractConfirmedDetailAcres(html, price) {
    try {
      if (!html || typeof html !== 'string') return null;
      ...
      const fromJsonLd = acresFromPriceMatchedJsonLd(scanHtml, price);
      if (fromJsonLd !== null) return fromJsonLd;

      return extractPriceAnchoredAcres(scanHtml, price);
    } catch (err) {
      return null;
    }
  }
  ...
  /**
   * True when a page is a client-rendered app shell that never hydrated — the
   * HTTP 200 a plain fetch gets from an SPA, carrying scripts and an empty
   * mount point but no listings.
   *
   * Deliberately requires BOTH signals: almost no visible text AND a framework
   * root element. A real markup-drift page (redesigned cards, same content)
   * still has paragraphs, nav text and footer copy, so it fails the first
   * test and is never mislabeled — the expensive mistake would be telling
   * Emma to add browser rendering when the actual fix is new selectors.
   */
  looksLikeUnrenderedShell(html) {
    ...
    const visibleText = html
      ...
      .replace(/\s+/g, ' ')
      .trim();
    if (visibleText.length > MAX_UNRENDERED_SHELL_TEXT_CHARS) return false;
    return SPA_ROOT_MARKERS.some(marker => marker.test(html));
  }
  ...
  async scrapeAll(counties) {
    ...
    for (const { url, county, state, page } of searchUrls) {
      ...
      // Circuit breaker (unsupported URL shapes): isolated 400s are tolerated
      // by design, but a run where they keep coming means the site changed its
      // URL scheme — a parser fix, not something to keep probing county by
      // county. Deliberately does NOT set abortedByBotWall: there is no
      // cooldown that makes a wrong URL right, so no post-cooldown retry.
      if (this.stats.unsupportedUrlPages >= UNSUPPORTED_URL_ABORT_THRESHOLD) {
        this.recordSourceIssue({
          type: 'site_abandoned',
          url: null,
          county: null,
          state: null,
          page: null,
          error: `Aborted after ${this.stats.unsupportedUrlPages} unsupported-URL (HTTP 400) pages — the site's URL scheme looks broken and needs a parser fix; remaining counties skipped this run`,
        });
        console.error(`[${this.name}] Aborting run: ${this.stats.unsupportedUrlPages} unsupported-URL pages — URL scheme likely changed`);
        break;
      }
      ...
      try {
        ...
        if (this.requiresBrowserRender) {
          ...
          // Search-page URLs are OURS: we build them, so a 400 means our URL
          // shape is wrong and a browser retry burns ~30s on a guaranteed
          // identical failure. Listing URLs fetched elsewhere (intake,
          // lead-recheck, detail enrichment) come from third parties, where a
          // 400 can be a fingerprint wall worth retrying — hence the default.
          html = await this.fetchPageSmart(url, { browserRetryOn400: false });
        }
        this.stats.checked++;
        ...
        // An unhydrated SPA shell also arrives as HTTP 200 parsing to zero
        // listings, and is indistinguishable from stale selectors by card
        // count alone — which is exactly how Whitetail's outage read: "fetched
        // OK, zero cards" for four straight nights, diagnosed as markup drift
        // with no way to confirm. If the page has no content but does have a
        // framework root element, the likely cause is that the site went
        // client-rendered (as NationalLand and MidwestLandGroup already are),
        // so give it one real-browser render before writing the page off.
        // Self-limiting: the signature is narrow, and the budget stops after a
        // few failures while letting a WORKING recovery run the whole site.
        if (parsed.length === 0
            && !this.requiresBrowserRender
            && this.looksLikeUnrenderedShell(html)
            && browserFetch.isEnabled()
            && (this.stats.shellRecoveries > 0 || this.stats.shellRetries < MAX_UNRENDERED_SHELL_RETRIES)) {
          this.stats.shellRetries++;
          try {
            html = await this.browserFetch(url);
            this._lastCardCount = null;
            parsed = this.parseSearchPage(html, county, state);
            rawCards = this._lastCardCount;
            if (parsed.length > 0) {
              this.stats.shellRecoveries++;
              console.log(`[${this.name}] Unrendered JS shell at ${url} — browser render recovered ${parsed.length} listings`);
            }
          } catch (browserErr) {
            if (browserErr.browserUnavailable) browserFetch.warnUnavailableOnce(this.name);
            // fall through — the shell HTML is classified below
          }
        }
        ...
        if (parsed.length === 0) {
          ...
          if ((page || 1) === 1 && cardsMatched === 0 && !this.looksLikeEmptyResults(html)) {
            // Name the two failure modes apart. "Selectors are stale" and "the
            // page never rendered" need opposite fixes — new selectors vs.
            // browser rendering — and reporting both as markup drift is what
            // left the Whitetail outage undiagnosable from the report alone.
            const unrenderedShell = this.looksLikeUnrenderedShell(html);
            this.stats.driftPages++;
            const savedTo = this.recordSourceIssue({
              type: unrenderedShell ? 'unrendered_shell' : 'markup_drift',
              url,
              county,
              state,
              page: page || null,
              error: unrenderedShell
                ? 'Page fetched OK but returned an unrendered JS shell (no content, framework root element present) — this source now needs browser rendering, not new selectors'
                // The link shapes are the diagnosis: they show what the
                // site's URLs look like now, so a changed detail-page scheme
                // is readable straight from the report instead of needing a
                // hand-captured page.
                : `Page fetched OK but zero listing cards matched and no "no results" marker found — selectors may be stale. Links on this page look like: ${describeLinkShapes(html) || '(none found)'}`,
            }, html);
            if (unrenderedShell) {
              console.warn(`[${this.name}] Unrendered JS shell at ${url} — needs browser rendering${savedTo ? ` (evidence: ${savedTo})` : ''}`);
            } else {
        ...
        this.stats.errors++;
        // Every failure exhausts the series: if page 1 of a county fails, its
        // deeper pages fail identically — walking them is waste. Only the
        // COUNTER differs, because each counter gates a different circuit
        // breaker above.
        exhaustedSeries.add(seriesKey);
        let issueType;
        if (browserFetch.isUnsupportedUrlStatus(err.status)) {
          // HTTP 400 = this URL SHAPE is unsupported (a filter or county
          // segment the site does not serve), not a block and not a site-wide
          // outage — the run's other URL shapes are unaffected. So it fails
          // its own series and is reported with evidence, but feeds NEITHER
          // circuit breaker. Counting these as blocked is what abandoned
          // LandWatch's whole remaining run on the Aug 6 and Aug 9 nightlies
          // over a handful of bad URLs.
          issueType = 'unsupported_url';
          this.stats.unsupportedUrlPages++;
        } else if (browserFetch.isBotWallStatus(err.status)) {
          issueType = 'blocked';
          this.stats.blockedPages++;
        } else {
          issueType = 'fetch_or_parse_error';
  ...
  prepareForRetry() {
    ...
    this.stats.unsupportedUrlPages = 0;
  ...
  paginationSeriesKey(url, county, state) {
    return [
      ...
      url
        ...
        // "/page-N" path segments are STRIPPED (not wildcarded) for the same
        // reason as the query params above: LandWatch's page 1 is the bare
        // county URL with NO segment while page 2 is /page-2 — a wildcard
        // would leave them as different keys and exhaustedSeries would never
        // skip a page-2 whose page 1 already failed. Land.com stamps /page-N
        // on every page including page 1, so stripping collapses it the same
        // way wildcarding did.
        .replace(/\/page-\d+\b/i, '')
  ...
  extractByDetailLinks($, opts) {
    ...
    const listings = [];
    // Per-call memo for elementVisibleText — see its doc comment.
    const textCache = new Map();

    // Drop non-rendered content BEFORE any text is read. Cheerio's .text()
    // includes <script> bodies, and modern listing pages embed a per-card
    // JSON-LD block inside the card markup — so the ancestor walk below was
    // reading raw JSON as "card text" and pulling acreage out of the
    // listing's DESCRIPTION PROSE instead of its visible acreage field.
    //
    // That silently dropped real listings. On the captured LandWatch
    // large-tract page, a $585,000 / 259-acre tract and a $1,137,000 /
    // 265-acre tract both render a clean "$585,000 • 259 acres" line, but
    // their JSON descriptions read "259.34 nrestricted acres" and
    // "264.95-acre property" — neither of which the acreage pattern matches —
    // so 2 of the county's 6 large tracts were discarded for "no acreage"
    // while a third only survived by the luck of its prose wording. Big
    // tracts are hit hardest: their descriptions favour exactly the
    // hyphenated "264.95-acre" phrasing.
    ...
    for (const [href, group] of hrefGroups) {
      ...
      for (const el of group) {
        // Check the anchor ITSELF before climbing: cards commonly wrap the
        // whole "$585,000 • 259 acres" line in the detail link, and that is
        // the tightest, least ambiguous card boundary there is.
        let $candidate = $(el);
        for (let depth = 0; depth <= 4; depth++) {
          if (depth > 0) $candidate = $candidate.parent();
          if (!$candidate.length) break;
          const candidateText = elementVisibleText($candidate.get(0), textCache);
   ...
   * and a leading-decimal figure like ".24 acres" (see lib/acreage.js — a
   * digit-required pattern here previously dropped the leading '.' and
   * silently turned 0.24 acres into 24 acres).
   */
  extractAcres(text) {
    const m = String(text).match(new RegExp(`(${ACREAGE_NUMBER})${ACREAGE_UNIT}`, 'i'));
    if (!m) return null;
    return parseAcreageNumber(m[1]);
  }
  ...
  /**
   * Parse acreage string like "160 acres", "160.5", or ".24 Acres" into a
   * number — routed through the shared parseAcreageNumber (lib/acreage.js)
   * so every parser's numeric parse agrees on leading-decimal handling.
   */
  parseAcres(acresStr) {
    if (!acresStr) return null;
    const cleaned = String(acresStr).replace(/[^0-9.]/g, '');
    return parseAcreageNumber(cleaned);
  }
...
/**
 * The most common link SHAPES on a page, as "/segment/segment (count)".
 *
 * When a source suddenly matches zero cards, the single most useful fact is
 * what its links look like NOW — a site that moved detail pages from
 * /hunting-land/{state}/{county}/{slug} to /property/{id} reports "zero cards"
 * either way, and without this the only way to tell is to capture the HTML by
 * hand and read it. Digits are collapsed to N so per-listing ids group
 * together instead of each counting once.
 */
function describeLinkShapes(html, limit = 5) {
  const counts = new Map();
  const hrefRe = /href=["'](\/[^"'#?]*)/gi;
  let m;
  let seen = 0;
  while ((m = hrefRe.exec(html)) !== null && seen++ < 4000) {
    const segments = m[1].split('/').filter(Boolean).slice(0, 3)
      .map(s => (/\d/.test(s) ? s.replace(/[0-9]+/g, 'N') : s));
...
/**
 * Visible text of an element with ELEMENT BOUNDARIES PRESERVED as spaces.
 *
 * Cheerio's .text() concatenates descendant text nodes with no separator, so
 * a card built from adjacent spans —
 *   <span>$585,000</span><span>259 acres</span><p>1330 Kentucky 790, ...</p>
 * — collapses to "$585,000 • 259 acres1330 Kentucky 790". The acreage pattern
 * ends in `acres?\b`, and there is no word boundary between "acres" and
 * "1330", so the match fails and the listing is discarded for having no
 * acreage. That is a silent, total loss of real inventory: on the captured
 * LandWatch large-tract page it threw away a $585,000 259-acre tract and a
 * $1,137,000 265-acre tract — a third of the county's large listings — and the
 * run reported nothing wrong, because a card that yields no acreage is simply
 * skipped.
 *
 * Joining on element boundaries is what the browser renders anyway, so this
 * reads the card the way a person looking at the page would.
 *
 * Results are memoized per call via a caller-supplied cache. The ancestor walk
 * asks for the text of steadily larger subtrees that share almost all of their
 * nodes, so without memoization a big search page re-walks the same DOM once
 * per anchor per depth — quadratic, and slow enough to stall a run. With the
 * cache each node's text is computed once and its parents concatenate the
 * cached pieces.
 */
function elementVisibleText(node, cache) {
  if (!node) return '';
  if (cache && cache.has(node)) return cache.get(node);

  let result = '';
  if (node.type === 'text') {
    result = String(node.data || '').replace(/\s+/g, ' ').trim();
  } else if (node.children && node.children.length) {
    const parts = [];
    for (const child of node.children) {
      const childText = elementVisibleText(child, cache);
      if (childText) parts.push(childText);
    }
    result = parts.join(' ');
  }

  if (cache) cache.set(node, result);
  return result;
}
...
/**
 * A schema.org QuantitativeValue node's numeric value, but ONLY when its unit
 * names acres — a bare number with no unit (or an sqft/hectare unit) must
 * never be read as acreage.
 */
function quantitativeValueAcres(value) {
  if (!value || typeof value !== 'object') return null;
  const unit = String(value.unitText || value.unitCode || '').toLowerCase();
  if (!/^acr(es?)?$/.test(unit)) return null;
  return parseAcreageNumber(value.value);
}

/**
 * Keys whose value IS the listing's acreage. Deliberately an allowlist rather
 * than a /acre/i substring test: 'pricePerAcre' contains "acre" but holds a
 * dollar figure, and reading it as acreage turned a real 105ac listing into a
 * 4000ac record. Any key naming a rate or a price is rejected outright, so a
 * future 'costPerAcreUsd'-style field can't slip through the allowlist either.
 */
const ACREAGE_KEY_RE = /^(?:acres?|acreage|acreageTotal|lotSizeAcres|totalAcres|landAcres|sizeAcres)$/i;
const PRICE_LIKE_KEY_RE = /per|price|cost|rate|usd|dollar/i;

function isAcreageKey(key) {
  const name = String(key || '');
  if (PRICE_LIKE_KEY_RE.test(name)) return false;
  return ACREAGE_KEY_RE.test(name);
}

/**
 * Depth-bounded walk of parsed JSON-LD for a listing's own structured
 * acreage — the most trustworthy signal there is, since it names the
 * LISTING's field rather than a number that happens to appear in nearby
 * description prose (the "1,000 acre Lake Halford" bug this whole extractor
 * exists to avoid). Two shapes are recognized: a directly-named numeric-ish
 * field ("acres", "acreage", "lotSizeAcres", ...) and a schema.org
 * QuantitativeValue node (lotSize/landArea) carrying an acre unit. Depth cap
 * mirrors findGeoInJson's, guarding against hostile deeply-nested JSON.
 */
function findAcresInJson(node, depth) {
  if (!node || typeof node !== 'object' || depth > 8) return null;
  if (Array.isArray(node)) {
    for (const item of node) {
      const found = findAcresInJson(item, depth + 1);
  ...
  for (const key of Object.keys(node)) {
    ...
    if (isAcreageKey(key)) {
      const direct = parseAcreageNumber(value);
      if (direct !== null && direct > 0) return direct;
      const qv = quantitativeValueAcres(value);
      if (qv !== null && qv > 0) return qv;
    }
    if (/^(lotsize|landarea)$/i.test(key)) {
      const qv = quantitativeValueAcres(value);
      if (qv !== null && qv > 0) return qv;
    }
  }
  for (const key of Object.keys(node)) {
    const found = findAcresInJson(node[key], depth + 1);
    if (found !== null) return found;
  }
  return null;
}
...
/**
 * JSON-LD acreage: extracts each <script type="application/ld+json"> block,
 * JSON.parses it (malformed blocks skipped), and walks it for a structured
 * acreage value. See findAcresInJson for the shapes recognized.
 */
function acresFromJsonLd(html) {
  const blockRe = /<script[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi;
  let m;
  let blocks = 0;
  while ((m = blockRe.exec(html)) !== null && blocks++ < 50) {
    let data;
    try { data = JSON.parse(m[1].trim()); } catch (err) { continue; }
    const acres = findAcresInJson(data, 0);
...
/**
 * JSON-LD acreage read ONLY from the node that carries the listing's own price
 * — the unambiguous signal extractConfirmedDetailAcres needs before it may
 * overwrite a card acreage.
 *
 * A page's JSON-LD often contains an ItemList of nearby/similar properties in
 * addition to the main entity, so "first acreage found" can belong to another
 * listing entirely. Matching on price pins the node to THIS property.
 */
function acresFromPriceMatchedJsonLd(html, price) {
  ...
  while ((m = blockRe.exec(html)) !== null && blocks++ < 50) {
    let data;
    try { data = JSON.parse(m[1].trim()); } catch (err) { continue; }
    const acres = findPriceMatchedAcresInJson(data, listingPrice, 0);
    if (acres !== null && acres > 0) return acres;
  }
  return null;
}
...
/**
 * Walk JSON-LD for a node that BOTH names a price matching the listing's and
 * carries an acreage. Only that pairing proves the acreage belongs to this
 * listing rather than to a sibling card in the same document.
 */
function findPriceMatchedAcresInJson(node, listingPrice, depth) {
  if (!node || typeof node !== 'object' || depth > 8) return null;
  if (Array.isArray(node)) {
    for (const item of node) {
      const found = findPriceMatchedAcresInJson(item, listingPrice, depth + 1);
      if (found !== null) return found;
    }
    return null;
  }

  if (nodeNamesPrice(node, listingPrice)) {
    // Search this node (and its children) for acreage, but do not let a nested
    // "nearby listings" collection inside it answer for the parent.
    const acres = findAcresInJson(node, depth);
    if (acres !== null && acres > 0) return acres;
  }

  for (const key of Object.keys(node)) {
    const found = findPriceMatchedAcresInJson(node[key], listingPrice, depth + 1);
    if (found !== null) return found;
  }
  return null;
}

/** True when this node (or its offers node) states the listing's price. */
function nodeNamesPrice(node, listingPrice) {
  const candidates = [node.price, node.Price];
  const offers = node.offers && typeof node.offers === 'object' ? node.offers : null;
  if (offers) {
    if (Array.isArray(offers)) offers.forEach(o => o && candidates.push(o.price));
    else candidates.push(offers.price);
  }
  return candidates.some(c => pricesMatch(parsePriceNumber(c), listingPrice));
}

/**
 * Bounded, DOM-free extraction of a detail page's <h1> text — the listing
 * title, where a bare "160 Acre Farm" style acreage figure often lives.
 * Scans only the leading slice, mirroring metaDescriptionByRegex.
 */
function headingText(html) {
  const head = String(html || '').slice(0, 200000);
  const m = /<h1[^>]*>([\s\S]*?)<\/h1>/i.exec(head);
  if (!m) return '';
