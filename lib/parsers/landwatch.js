...
class LandWatchParser extends BaseParser {
  ...
  buildSearchUrls(counties) {
    ...
    for (const { county, state } of counties) {
      ...
      // main pages. acres-over-{N} with an arbitrary N is UNCONFIRMED on a
      // county page (indexed examples are fixed buckets on state pages); if
      // the site 400s it, the error-shell block detection reports it as
      // blocked — never as markup drift — and the evidence queue's variant
      // captures will settle the real segment format.
      urls.push({
        url: `${countyBase}/acres-over-${largeTractMin}`,
        county,
        state,
        page: 1,
      });
