...
class MidwestLandGroupParser extends BaseParser {
  constructor() {
    ...
    // CONSTRAINT: this site has NO per-county/per-state URL — buildSearchUrls
    // returns ONE national index regardless of the county list (see class doc).
    // County rotation therefore does nothing useful here: the same single page
    // is fetched no matter which county subset is passed, so a rotation would
    // only shrink the downstream county filter's target set (hiding leads) for
    // zero reduction in requests. resolveParserCounties (lib/scraper.js) reads
    // this flag to hand such parsers the full county list, unrotated.
    this.usesCountyUrls = false;
    // CONSTRAINT: the /listings/ index renders its cards CLIENT-SIDE — a plain
    // HTTP fetch returns only the hidden JS card template with zero real
    // listings (the empty skeleton the zero-parse path would mis-diagnose as
    // markup drift). So scrapeAll must fetch this source through the real
    // browser; requiresBrowserRender routes it there and, when no browser is
    // available, skips the site with an explanatory issue instead.
