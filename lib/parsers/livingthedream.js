...
class LivingTheDreamParser extends BaseParser {
  ...
  parseSearchPage(html, defaultCounty, state) {
    ...
    const targetCounties = new Set(
      getCountiesForState(state).map(c => c.county.toLowerCase())
    );

    $('.property-listing, .listing-item, .property-card').each((_, el) => {
      try {
        const $el = $(el);
  ...
  extractCounty(text, targetCounties) {
    ...
    for (const county of targetCounties) {
      if (lower.includes(county)) return county.charAt(0).toUpperCase() + county.slice(1);
    }
    // Try "County Name County" pattern
    const match = text.match(/(\w[\w\s]*?)\s*county/i);
    if (match) {
      const name = match[1].trim().toLowerCase();
      if (targetCounties.has(name)) return name.charAt(0).toUpperCase() + name.slice(1);
    }
    return null;
  }
}
