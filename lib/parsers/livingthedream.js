 ...
 *
 * IMPORTANT — the site covers only FIVE states (its own title:
 * "Land For Sale | MO, KY, KS, AR, IL"). buildSearchUrls therefore sweeps only
 * target states inside that coverage set — a `/land-for-sale/oklahoma/` request
 * would just 404 — and dedupes to ONE series per covered state. A state page
...
// The site covers only these five states (its own page title:
// "Land For Sale | MO, KY, KS, AR, IL"). Requesting a state outside this set
// would just 404, so buildSearchUrls skips non-covered target states.
const COVERED_STATES = new Set(['MO', 'KY', 'KS', 'AR', 'IL']);
