...
// Fallback header figure when a review report predates largeTractHighlightMin
// (e.g. an old cached report or a hand-built test report without the field).
const DEFAULT_LARGE_TRACT_HIGHLIGHT_MIN_DISPLAY = 500;
...
/**
 * Sort comparator for any per-listing "details" list (duplicateDetails,
 * skippedUnavailable, ...): biggest tract first. Emma wants large listings
 * surfaced at the top of a list rather than buried under a pile of small
 * ones — a missing/unknown acreage sorts last, not first.
 */
function byAcresDesc(a, b) {
...
function buildScraperBody(scraperReport, priceCheckReport, date, reviewReport, intakeReport, options = {}) {
  ...
  for (const [site, stats] of Object.entries(scraperReport.sites)) {
    if (stats.status === 'error') {
      ...
      // Distinct per-site accounting for the two post-enrichment safety-net
      // checks (lib/scraper.js Step 4.6) — count is enough here; the full
      // name/phrase/URL detail for skipped-unavailable listings is in its own
      // section below.
      if (stats.rejectedBelowMinAcres > 0) {
        lines.push(`    (${stats.rejectedBelowMinAcres} rejected: below minimum acreage)`);
      }
      if (stats.skippedUnavailable > 0) {
        lines.push(`    (${stats.skippedUnavailable} skipped: already unavailable — see SKIPPED section below)`);
      }
    }
  }
  lines.push('');
  ...
  lines.push(`  TOTALS: ${totalWriteText}, ${scraperReport.totals.duplicates} dupes, ${scraperReport.totals.rejected} rejected, ${scraperReport.totals.skippedUnavailable || 0} skipped (already unavailable)`);
  if ((scraperReport.totals.rejectedBelowMinAcres || 0) > 0) {
    lines.push(`    (${scraperReport.totals.rejectedBelowMinAcres} of the rejected were below the minimum acreage floor)`);
  }
  ...
  // Big listings the run SAW but did not write, each with the reason it was
  // dropped. Without this the answer to "why do the really large properties
  // never reach me?" is buried in an anonymous rejected count.
  const largeNotWritten = Array.isArray(scraperReport.largeTractsNotWritten)
    ? scraperReport.largeTractsNotWritten
    : [];
  if (largeNotWritten.length > 0) {
    const threshold = largeNotWritten.reduce((min, l) => Math.min(min, l.acres), Infinity);
    lines.push(`LARGE TRACTS FOUND BUT NOT WRITTEN (${largeNotWritten.length})`);
    lines.push('-'.repeat(30));
    lines.push('  These were parsed successfully, then dropped by the filter.');
    lines.push('  If the same reason repeats, the filter — not the sources — is the reason');
    lines.push('  big properties are missing.');
    lines.push('');
    for (const l of [...largeNotWritten].sort((a, b) => b.acres - a.acres)) {
      const where = [l.county, l.state].filter(Boolean).join(', ');
      const priceText = l.price ? `$${Number(l.price).toLocaleString()}` : 'no price';
      const cpa = l.price && l.acres ? ` ($${Math.round(l.price / l.acres)}/ac)` : '';
      lines.push(`  ${Math.round(l.acres)}ac — ${priceText}${cpa} — ${where || 'unknown county'} [${l.source || '?'}]`);
      lines.push(`    ${l.reason}`);
      if (l.url) lines.push(`    ${l.url}`);
    }
    // Group the reasons so a systematic cause is obvious at a glance.
    const byReason = new Map();
    for (const l of largeNotWritten) {
      const key = String(l.reason || '').split(':')[0].trim() || 'rejected';
    ...
    if (Number.isFinite(threshold)) {
      lines.push(`  (listing everything at or above ${Math.round(threshold)} acres that was dropped)`);
    }
    lines.push('');
  }
  ...
  if (intakeReport && (intakeReport.processed > 0 || intakeReport.loadError)) {
    ...
    if (intakeReport.loadError) {
      ...
      lines.push(`  Processed: ${intakeReport.processed} | Added: ${intakeReport.created} | Duplicates: ${intakeReport.duplicates} | Rejected: ${intakeReport.rejected || 0}`);
      ...
      const rejections = intakeReport.rejections || [];
      if (rejections.length > 0) {
        lines.push(`  🚫 NOT IMPORTED (${rejections.length}):`);
        for (const r of rejections) {
          lines.push(`     ${r.url} (${r.submitter})`);
          lines.push(`       Not imported: ${r.reason}`);
        }
      }
  ...
  // Nightly lead recheck (re-fetches each New Lead / Emma Review record's own
  // URL — see lib/lead-recheck.js). Report only; never changes Stage.
  const leadRecheckReport = options.leadRecheckReport || null;
  if (leadRecheckReport) {
    lines.push('LEAD RECHECK');
    lines.push('-'.repeat(30));
    lines.push(...buildLeadRecheckSection(leadRecheckReport));
  } else if (scraperReport.leadRecheckError) {
    lines.push('⚠️ LEAD RECHECK: FAILED');
    lines.push('-'.repeat(30));
    lines.push(`  ${scraperReport.leadRecheckError}`);
    lines.push('  New Lead / Emma Review records were NOT rechecked this run.');
    lines.push('');
  }

  // Skipped-unavailable listings (under contract/pending/off-market/...)
  // caught AFTER detail enrichment, right before the would-be Airtable write
  // — mirrors how duplicateDetails is collected (lib/scraper.js) and
  // rendered below. Emma wants these visible by name/matched phrase/URL, not
  // silently dropped: a real case handed her a lead that was already under
  // contract with nothing in the pipeline ever surfacing that. Biggest
  // tracts first (byAcresDesc) — she fears large listings get buried.
  const skippedUnavailable = [...(scraperReport.skippedUnavailable || [])].sort(byAcresDesc);
  if (skippedUnavailable.length > 0) {
    lines.push('SKIPPED — LISTING UNAVAILABLE (under contract/pending/off-market)');
    lines.push('-'.repeat(30));
    for (const s of skippedUnavailable.slice(0, 20)) {
      lines.push(`  ${s.source}: ${s.name} — matched "${s.phrase}"`);
      lines.push(`    ${s.url}`);
    }
    if (skippedUnavailable.length > 20) {
      lines.push(`  ... and ${skippedUnavailable.length - 20} more`);
    }
    lines.push('');
  }

  // Cross-site duplicates caught (summarize). Biggest tracts first — same
  // reasoning as the skipped-unavailable list above.
  const crossSiteDupes = scraperReport.duplicateDetails
    .filter(d => d.matchType === 'fingerprint' || d.matchType === 'session-fingerprint' || d.matchType === 'location-price')
    .sort(byAcresDesc);
  ...
  if (scraperReport.sourceIssues && scraperReport.sourceIssues.length > 0) {
    ...
    const unsupportedUrl = scraperReport.sourceIssues.filter(i => i.type === 'unsupported_url');
    if (unsupportedUrl.length > 0) {
      lines.push(`  🔗 ${unsupportedUrl.length} page(s) hit an unsupported URL shape (parser fix needed — not a bot block)`);
    }
    const shells = scraperReport.sourceIssues.filter(i => i.type === 'unrendered_shell');
    if (shells.length > 0) {
      lines.push(`  🖥️ ${shells.length} page(s) returned an unrendered JS shell — that source went client-rendered and needs browser rendering (NOT stale selectors)`);
    }
...
/**
 * "LEAD RECHECK" section lines, shared between the standalone renderer here
 * and buildScraperBody. loadError means the whole step never got past
 * loading candidate records (see lib/lead-recheck.js) and is surfaced like
 * any other failed section rather than silently rendering "0 checked".
 */
function buildLeadRecheckSection(report) {
  const lines = [];
  if (report.loadError) {
    lines.push(`  ⚠️ Could not load candidate leads: ${report.loadError}`);
    lines.push('');
    return lines;
  }

  lines.push(`  Rechecked: ${report.checked} of ${report.totalCandidates} candidate lead(s) (New Lead / Emma Review)`);
  if (report.skippedNoUrl > 0) {
    lines.push(`  Skipped (no URL on record): ${report.skippedNoUrl}`);
  }
  if (report.fetchFailed > 0) {
    lines.push(`  ⚠️ Fetch failures: ${report.fetchFailed} (not counted as findings — could not verify)`);
  }
  if (report.droppedByCap > 0) {
    const names = report.droppedNames.slice(0, 10).join(', ');
    lines.push(`  ⏳ Nightly cap hit — ${report.droppedByCap} lead(s) deferred to a later night: ${names}${report.droppedByCap > 10 ? ', ...' : ''}`);
  }
  lines.push('');

  if (report.underContract.length === 0 && report.acreageMismatches.length === 0) {
    lines.push(`  ✅ All ${report.checked} rechecked leads look live.`);
    lines.push('');
    return lines;
  }

  if (report.underContract.length > 0) {
    lines.push('  🚫 NOW UNDER CONTRACT / SOLD / OFF MARKET');
    for (const u of report.underContract) {
      lines.push(`    ${u.name} (${u.stage || 'unknown stage'}) — matched "${u.phrase}"`);
      lines.push(`      ${u.url}`);
    }
    lines.push('');
  }

  if (report.acreageMismatches.length > 0) {
    lines.push('  📏 ACREAGE MISMATCHES (recorded vs. live)');
    for (const m of report.acreageMismatches) {
      lines.push(`    ${m.name}: recorded ${m.recordedAcres}ac vs live ${m.liveAcres}ac`);
      lines.push(`      ${m.url}`);
    }
    lines.push('');
  }
...
function buildReviewSection(report) {
  ...
  // LARGE TRACTS — every lead reviewed this run at/above
  // review.largeTractHighlightMin (default 500ac), regardless of score.
  // Big tracts are scored down (no sweet-spot bonus past 400ac) and rarely
  // make the standout list, so this section is the only place Emma sees
  // them; entries here are additive to, not instead of, any standout line.
  if (report.largeTracts && report.largeTracts.length > 0) {
    const min = report.largeTractHighlightMin || DEFAULT_LARGE_TRACT_HIGHLIGHT_MIN_DISPLAY;
    lines.push(`  🌳 LARGE TRACTS (${min}ac+)`);
    for (const t of report.largeTracts) {
      lines.push(`    ${t.name}`);
      lines.push(`      ${t.county} County, ${t.state} | ${t.acres}ac | $${t.price?.toLocaleString()} ($${Math.round(t.cpa)}/ac)`);
    }
    lines.push('');
  }
