...
function buildScraperSubject(report) {
  ...
  if (report.dryRun) {
    ...
    return `${prefix}CCL Scraper (dry run) — ${totals.wouldWrite || 0} would write${hasErrors ? ` — ${totals.errors} errors` : ''}`;
  }
  if (hasErrors && totals.written === 0) {
    return `⚠️ CCL Scraper — ERRORS — ${totals.errors} failures`;
  }
  if (hasErrors) {
    return `⚠️ CCL Scraper — ${totals.written} new leads, ${totals.errors} errors`;
  }
  if (totals.written === 0) {
...
async function pingHealthcheck(success) {
  ...
  if (!url) return;
  const target = success ? url : `${url.replace(/\/$/, '')}/fail`;
  try {
