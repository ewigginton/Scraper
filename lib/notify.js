 ...
 */
async function sendScraperEmail(scraperReport, priceCheckReport, reviewReport, intakeReport) {
  const to = process.env.EMAIL_TO;
  const date = new Date().toLocaleDateString('en-US', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
  });

  const subject = buildScraperSubject(scraperReport, reviewReport, intakeReport);
  const body = buildScraperBody(scraperReport, priceCheckReport, date, reviewReport, intakeReport);
...
function buildScraperSubject(report, reviewReport, intakeReport) {
  const { totals } = report;
...
function buildScraperBody(scraperReport, priceCheckReport, date, reviewReport, intakeReport) {
  const lines = [];
