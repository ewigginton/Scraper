...
/**
 * Separator between an acreage figure and its unit word, covering the
 * hyphenated adjective form as well as the plain one: "40 acres", "40acres",
 * "201.15+/- acre", "±40 acres" and — the one that was missing — "1,200-acre
 * ranch". Listing prose uses the hyphenated form precisely when the acreage
 * modifies a noun, which is overwhelmingly how BIG tracts get described
 * ("a 264.95-acre property", "this 1,200-acre ranch"), so not matching it lost
 * large listings specifically.
 */
const ACREAGE_UNIT = '\\s*(?:±|\\+\\/-)?[\\s-]*acres?\\b';
...
const PRICE_BULLET_ACRES_RE = new RegExp(
  `\\$\\s?[\\d,]+(?:\\.\\d+)?\\s*[•·|]\\s*(${ACREAGE_NUMBER})${ACREAGE_UNIT}`, 'i'
);
const VIEW_ACRES_PRICED_RE = new RegExp(
  `\\bview\\s+(${ACREAGE_NUMBER})${ACREAGE_UNIT}\\s+priced\\s+at\\b`, 'i'
);
...
const PRICE_BULLET_ACRES_ANCHORED_RE = new RegExp(
  `\\$\\s?([\\d,]+(?:\\.\\d+)?)\\s*[•·|]\\s*(${ACREAGE_NUMBER})${ACREAGE_UNIT}`, 'gi'
);
const VIEW_ACRES_PRICED_ANCHORED_RE = new RegExp(
  `\\bview\\s+(${ACREAGE_NUMBER})${ACREAGE_UNIT}\\s+priced\\s+at\\s*\\$\\s?([\\d,]+(?:\\.\\d+)?)`, 'gi'
);
