...
// 400 is NOT one of them: on the CoStar network it means "unsupported URL
// shape", which is a parser bug, not a block (see isUnsupportedUrlStatus).
...
function diagnoseSite(siteName, siteReport, sourceIssues) {
  ...
  // 1b. HTTP 400 storm — the CoStar network answers an unsupported URL SHAPE
  // with 400, so a dominant 400 means the parser is asking for a filter or
  // county segment the site does not serve. Deliberately separate from the
  // bot-wall verdict below: these no longer count as blocks (they abandoned
  // whole runs when they did — see lib/parsers/base-parser.js), and the fix is
  // a URL-builder change, not a wait-for-the-wall-to-lift.
  if (dominant && dominant.category === 'unsupported_url' && dominant.ratio >= DOMINANCE_THRESHOLD) {
    const fraction = describeFraction(dominant.count, totalRequests, 'request');
    const stopSuffix = stoppedAfter !== null ? ` — stopped after ${stoppedAfter} requests` : '';
    return {
      severity: 'action_needed',
      headline: `${fraction} returned HTTP 400 — the site rejects those URL shapes; the parser's URL builder needs updating${stopSuffix}`,
    };
  }

  // 2. Bot wall — 403/429/503 or a detected challenge page dominate.
...
function tallyByCategory(issues) {
  const counts = { not_found: 0, unsupported_url: 0, bot_wall: 0, drift: 0, server_error: 0, other_error: 0 };
