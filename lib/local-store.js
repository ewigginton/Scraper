...
function leadRecheckStatePath() {
  return path.join(getDataDir(), 'lead-recheck', 'state.json');
}

/**
 * Load the nightly lead-recheck "last checked" state: a plain
 * { [airtableRecordId]: isoTimestamp } map (lib/lead-recheck.js) used to pick
 * the oldest-unchecked leads first when the nightly cap is hit. Tolerates a
 * missing/corrupt file — recheck must never crash the nightly run over a bad
 * local cache; an empty map just means every candidate looks "never checked"
 * (oldest) until this run's own updates are saved.
 */
function loadLeadRecheckState() {
  try {
    const raw = fs.readFileSync(leadRecheckStatePath(), 'utf8');
    const parsed = JSON.parse(raw);
    return (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : {};
  } catch (err) {
    if (err.code !== 'ENOENT') {
      console.warn(`[LocalStore] Could not read lead-recheck state: ${err.message}`);
    }
    return {};
  }
}
...
/**
 * Persist the lead-recheck "last checked" state (see loadLeadRecheckState).
 * Whole-file overwrite — the caller passes the full merged/pruned state, not
 * a delta.
 */
function saveLeadRecheckState(state) {
  const dir = path.join(getDataDir(), 'lead-recheck');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(leadRecheckStatePath(), JSON.stringify(state), 'utf8');
}
...
module.exports = {
  ...
  loadCoverageLog,
  loadLeadRecheckState,
  saveLeadRecheckState,
};
