...
// Canonical source for availability-status phrases and the whole-word
// matcher — shared with lib/intake.js and lib/scraper.js so all three paths
// use the exact same detection, including the anchored 'sold' patterns that
// live only behind findAvailabilityMatches (not in AVAILABILITY_FLAGS
// itself). See lib/availability.js.
const { AVAILABILITY_FLAGS, matchesKeyword, findAvailabilityMatches } = require('./availability');
...
// Availability-status phrases (AVAILABILITY_FLAGS, imported above from
// lib/availability.js): the listing itself is no longer available (or may
// not be) — under contract, pending, sold, etc. This is NEVER a property
// dealbreaker (the tract itself may be fine) and must NEVER auto-reject a
// lead here (status text is often stale, or a false hit elsewhere on the
// page) — Emma decides. It still needs its own visible flag: a real case
// handed her a Whitetail listing whose detail page plainly said "under
// contract" and nothing in the pipeline ever surfaced that. (lib/scraper.js
// takes a harder line: a FRESH listing that already matches one of these
// phrases is skipped before it's ever written, rather than flagged.)

// Fallback when config/settings.json has no review.largeTractHighlightMin.
const DEFAULT_LARGE_TRACT_HIGHLIGHT_MIN = 500;
...
async function runReview() {
  ...
  // Emma never sees 500ac+ properties in the standout list — review scoring
  // demotes >400ac (no sweet-spot bonus), so big tracts rarely score high
  // enough to surface there. This dedicated section lists every reviewed
  // lead at/above the threshold regardless of score, so she still sees them.
  const largeTractHighlightMin = (settings.review && settings.review.largeTractHighlightMin)
    || DEFAULT_LARGE_TRACT_HIGHLIGHT_MIN;
  ...
  // Everything parked in New Lead, Price Drop (legacy — promotions now land
  // in New Lead), or Emma Review is waiting on a human decision. Listing
  // these with their age makes "1 new" traceable to an actual Airtable
  // record instead of looking like a phantom lead.
  report.awaiting = buildAwaitingList([...newLeads, ...priceDrop, ...emmaReview]);
  ...
  for (const record of allToReview) {
    try {
      ...
      // Flag likely dealbreakers in the notes and the email — but NEVER
      // change the Stage. Every lead stays 'New Lead' until Emma rates it;
      // her triage filter depends on new records arriving (and staying)
      // there. Auto-moving flagged records to 'Emma Review' made them vanish
      // from her New Lead view before she ever saw them.
      if (analysis.dealbreakers.length > 0) {
        report.flagged.push({
          name: record.fields.Name,
          flags: analysis.dealbreakers,
        });
        console.log(`[Review] DEALBREAKER FLAGS: ${record.fields.Name} — ${analysis.dealbreakers[0]}`);
      }
      // Track standouts
      ...
      // LARGE TRACTS: every lead at/above the threshold, regardless of score
      // or which branch above it landed in — additive, never a replacement
      // for the standout list.
      if ((record.fields[FIELDS.acres] || 0) >= largeTractHighlightMin) {
        report.largeTracts.push({
          name: record.fields.Name,
          county: loc?.county,
          state: loc?.state,
          price: record.fields[FIELDS.price],
      ...
      // Availability status (under contract/pending/sold/...) is a parallel
      // path, independent of the dealbreaker/standout/yellow-flag branches
      // above: it must reach report.flagged (the email's '⚠️ FLAGGED' lines)
      // no matter which of those branches fired, but it never blocks a
      // standout and never becomes a dealbreaker or an auto-reject — status
      // text can be stale, so Emma makes the call. Merge into an existing
      // flagged entry for this record rather than duplicating a row.
      if (analysis.availabilityFlags.length > 0) {
        const existingFlagRow = report.flagged.find(f => f.name === record.fields.Name);
        if (existingFlagRow) {
          existingFlagRow.flags = [...existingFlagRow.flags, ...analysis.availabilityFlags];
        } else {
          report.flagged.push({
            name: record.fields.Name,
            flags: analysis.availabilityFlags,
          });
        }
        console.log(`[Review] AVAILABILITY FLAG: ${record.fields.Name} — ${analysis.availabilityFlags[0]}`);
      }
...
function analyzeLead(record, enrichment = {}) {
  ...
  // Same text as dealbreakers/yellowFlags — availability status lives in the
  // same description/notes prose. Labeled distinctly from yellowFlags so
  // callers can surface it without treating it as a property-quality signal.
  // Goes through findAvailabilityMatches (not a manual AVAILABILITY_FLAGS
  // filter) so the anchored 'sold' detection applies here too.
  const availabilityFlags = findAvailabilityMatches(text)
    .map(kw => `listed as ${kw}`);
...
function formatAnalysis(analysis) {
  ...
  // Availability status — surfaced ahead of dealbreakers since "already
  // under contract" outranks every other consideration for Emma's triage.
  if (analysis.availabilityFlags.length > 0) {
    lines.push(`\n🚩 AVAILABILITY:`);
    for (const flag of analysis.availabilityFlags) lines.push(`  - ${flag}`);
  }
...
module.exports = { runReview, analyzeLead, matchesKeyword, AVAILABILITY_FLAGS };
