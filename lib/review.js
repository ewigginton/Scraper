...
// Availability-status phrases: the listing itself is no longer available (or
// may not be) — under contract, pending, sold, etc. This is NEVER a property
// dealbreaker (the tract itself may be fine) and must NEVER auto-reject a
// lead (status text is often stale, or a false hit elsewhere on the page) —
// Emma decides. It still needs its own visible flag: a real case handed her
// a Whitetail listing whose detail page plainly said "under contract" and
// nothing in the pipeline ever surfaced that.
// Only SPECIFIC availability phrases — the bare words 'sold' and 'contingent'
// were removed because they false-positive on ordinary comp prose ("similar
// tracts have sold for $3,000/acre", "access contingent upon a survey"), which
// describes neighboring sales or closing conditions, not THIS listing's status.
...
function analyzeLead(record, enrichment = {}) {
  ...
  const availabilityFlags = AVAILABILITY_FLAGS
    .filter(kw => matchesKeyword(text, kw))
    .map(kw => `listed as ${kw}`);
