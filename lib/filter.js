...
// Build a fast lookup map: "county|state" -> maxCPA
...
/**
 * Get all target counties for a state.
 */
function getCountiesForState(state) {
  const stateUpper = (state || '').toUpperCase();
  return countyData.counties.filter(c => c.state.toUpperCase() === stateUpper);
}

/**
 * Get all unique target states.
 */
function getTargetStates() {
  return [...new Set(countyData.counties.map(c => c.state.toUpperCase()))];
}
