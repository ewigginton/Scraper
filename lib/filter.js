...
function normalizedKey(county, state) {
  ...
  // Spaces are removed, not just collapsed: a hand-entered Airtable target
  // ("LeFlore") and the spelling a listing card carries ("Le Flore") are the
  // same county, and a target that fails to resolve here rejects every lead
  // in it as an untargeted county.
  return `${normCounty.replace(/\s+/g, '')}|${normState.toUpperCase()}`;
}
