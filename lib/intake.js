...
// sold, etc.) — lib/review.js owns the list so the scraper/review path and
// this intake path never drift apart.
const { AVAILABILITY_FLAGS, matchesKeyword } = require('./review');
  ...
  for (const record of queue) {
    ...
    try {
      ...
      if (dupCheck.isDuplicate) {
        ...
        // Cross-site duplicate resolved to an existing Airtable record: merge
        // this submission's source+URL into it instead of just discarding it
        // (listing.source is still the 'Intake: host (submitter)' format).
        // A throw here must not fail the intake item — fall back to the plain
        // Duplicate result above with the error noted.
        if (dupCheck.existingRecord && url) {
          try {
            const mergeResult = await airtable.mergeSourceIntoRecord(dupCheck.existingRecord, listing);
            if (mergeResult && mergeResult.merged) {
              report.sourceMerges++;
              report.mergedDetails.push({ url, submitter, reason: dupCheck.reason });
              updateFields = {
                [F.status]: S.duplicate,
                [F.result]: `Already in the Land table — added this URL as an additional source on the existing lead (${dupCheck.reason}).`,
                [F.landRecord]: [dupCheck.existingRecord.id],
                [F.processedAt]: new Date().toISOString(),
              };
            }
            // mergeResult.merged === false → URL already recorded on the
            // existing record; fall through to the plain duplicate result.
          } catch (mergeErr) {
            console.warn(`[Intake] Could not merge source into ${dupCheck.existingRecord.id}: ${mergeErr.message}`);
            updateFields[F.result] = `Already in the Land table — ${dupCheck.reason} (could not add as an additional source: ${mergeErr.message})`;
          }
        }

        await airtable.updateIntakeRecord(record.id, updateFields);
...
function extractListingDetails(html, url) {
  ...
  // Same fetched-page text the other fields are pulled from — scanned with
  // the exact keyword list lib/review.js uses on scraped/enriched leads, so
  // an "under contract" page never slips through just because it arrived via
  // Listing Intake instead of the scraper.
  const availabilityFlags = AVAILABILITY_FLAGS.filter(kw => matchesKeyword(combinedText, kw));
...
function buildNotes(extracted, submitterNote, submitter) {
  ...
  // Availability-status phrase goes straight into notes (→ FIELDS.notes) so
  // the record — created regardless, Emma decides — also gets caught by
  // analyzeLead's own AVAILABILITY_FLAGS scan on review night, exactly like a
  // scraped/enriched lead would.
  if (extracted.availabilityFlags && extracted.availabilityFlags.length > 0) {
    lines.push('', `⚠️ Page text says "${extracted.availabilityFlags[0]}" — availability may have changed since this URL was submitted.`);
  }
...
function describeCreated(extracted) {
  ...
  let summary = `Created New Lead from submitted URL: ${parts.join(', ')}.`;
  if (extracted.availabilityFlags && extracted.availabilityFlags.length > 0) {
    summary += ` ⚠️ Page text says "${extracted.availabilityFlags[0]}" — please verify it's still available before contacting the seller.`;
  }
  return summary;
}
