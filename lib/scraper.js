...
async function runScraper(options = {}) {
  ...
  const ctx = { dedupIndex, sessionFingerprints, report, dryRun };
  ...
  for (let i = 0; i < parsers.length; i += maxConcurrentSites) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      // the other sites have finished their own first passes
...
async function retryAbortedSites(abortedParsers, cooldownMinutes, ctx, counties) {
  ...
  for (const { parser, abortedAt, firstPass } of abortedParsers) {
    ...
    let retryListings;
    try {
      retryListings = await parser.scrapeAll(counties);
    } catch (err) {
