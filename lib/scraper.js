...
async function processScrapedListings(parser, listings, ctx) {
  ...
  // fuller description. ONLY the to-be-written leads get a detail fetch
  // (0-20/night) — dupes and rejects never do. The full description is where
  // DEALBREAKER language (e.g. 'conservation easement') lives; the ~400-char
  // search-card blurb rarely carries it, so review scanning only the blurb
  // misses those. Failures are non-fatal — the lead is still written with the
  // card blurb (see enrichListingsWithDetail).
 ...
 * into listing.description (which lands in the Land table's 'Scraper Notes'
 * field — the exact text lib/review.js scans for DEALBREAKER/flag keywords).
 * Uses the same jittered request cadence as scrapeAll so enrichment fetches
 * don't form a metronomic bot signature. Every failure is non-fatal: it's
 * logged, counted (siteReport.enrichmentFailed), and the lead is written with
 * its original card blurb.
 */
async function enrichListingsWithDetail(parser, listings, siteReport) {
  ...
  for (const listing of listings) {
    ...
    try {
      ...
      const detailText = parser.extractDetailDescription(html);
