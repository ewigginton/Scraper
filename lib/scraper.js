...
// Fallback when config/settings.json has no review.largeTractHighlightMin —
// the acreage at or above which a listing the run DIDN'T write gets itemized
// with its reason (see recordLargeTractRejection).
const LARGE_TRACT_REPORT_MIN_ACRES = 500;
// The email lists these individually, so cap the collection. A run producing
// more than this has a systemic problem the summary line already reveals.
const MAX_LARGE_TRACT_REPORT_ROWS = 50;
...
async function processScrapedListings(parser, listings, ctx) {
  ...
  // NOTE: siteReport.sourceIssues is copied into report.sourceIssues further
  // down (after Step 3.5/4.5 detail enrichment) rather than here — enrichment
  // can append its own issues (e.g. an acreage-mismatch suspicion) to this
  // same array, and copying too early would silently drop them from the email.
  ...
  for (const listing of listings) {
    ...
    if (!filterResult.passed) {
      ...
      recordLargeTractRejection(report, parser, listing, filterResult.reason);
      continue;
    }
  ...
  // Copy this site's full sourceIssues (first pass + anything Step 3.5/4.5
  // enrichment appended, e.g. an acreage-mismatch suspicion) into the run
  // report now that every step has had its chance to add to it.
...
// Every field EXCEPT acres gets the plain "fill only if missing" treatment
// below — acres additionally gets a disagreement cross-check (see the acreage
// block in applyDetailToListing), so it's handled separately from this loop.
const FILL_IF_MISSING_FIELDS = FILTER_CRITICAL_FIELDS.filter(field => field !== 'acres');

// A card's regex-guessed acreage and the detail page's structured acreage
// disagreeing by at least this multiple is never innocent rounding — it's the
// "N-acre lake/park/development" bug (a description phrase like "on the
// gorgeous 1,000 acre Lake Halford" hijacking a 0.94ac card's true acreage).
// The detail page's structured value is trusted at or above this ratio.
const ACREAGE_MISMATCH_RATIO = 10;
 ...
 */
/**
 * Record a BIG listing that the run parsed but did not write, with the reason.
 *
 * Emma's standing report: the really large properties never arrive, even in
 * runs where every source is healthy. The counters could not answer why — a
 * 1,200-acre tract dropped for "Missing or invalid price" or for being 45%
 * over the county's CPA target landed in the same anonymous "N rejected"
 * total as a 3-acre lot, so a systematic loss of exactly the listings she
 * cares most about was indistinguishable from normal filtering noise.
 *
 * This is REPORT-ONLY — it never changes a filter decision, only makes the
 * decision visible, so a real pattern ("every big tract is priced on request"
 * / "they all sit 40% over target") can be acted on with evidence instead of
 * guessed at.
 */
function recordLargeTractRejection(report, parser, listing, reason) {
  const min = (settings.review && settings.review.largeTractHighlightMin)
    || LARGE_TRACT_REPORT_MIN_ACRES;
  const acres = Number(listing.acres);
  if (!Number.isFinite(acres) || acres < min) return;
  if (!Array.isArray(report.largeTractsNotWritten)) report.largeTractsNotWritten = [];
  if (report.largeTractsNotWritten.length >= MAX_LARGE_TRACT_REPORT_ROWS) return;
  report.largeTractsNotWritten.push({
    source: parser ? parser.name : null,
...
function applyDetailToListing(parser, listing, html) {
  ...
  // Acreage: prefer the parser's own structured detail read (parseDetailPage,
  // e.g. LANDFLIP's "with N acres" description parse); fall back to the
  // generic structured extractor (JSON-LD / price-adjacent text / heading —
  // see BaseParser#extractDetailAcres) when the parser has none.
  //   - Missing card acreage is simply filled, as with the other fields above.
  //   - A card acreage that DISAGREES with the detail acreage by >=10x is
  //     overridden — the detail page's structured signal is far more
  //     trustworthy than a card-text regex that can snag a nearby "N acre
  //     lake/park" phrase — and the mismatch is recorded as a source-health
  //     suspicion so it surfaces in the nightly report instead of silently
  //     correcting undetected.
  const rawDetailAcres = detail.acres != null && detail.acres !== '' ? detail.acres : null;
  const detailAcres = rawDetailAcres !== null ? rawDetailAcres : parser.extractDetailAcres(html);
  if (detailAcres != null && detailAcres > 0) {
    if (!listing.acres) {
      listing.acres = detailAcres;
    } else if (listing.acres !== detailAcres) {
      const ratio = Math.max(listing.acres, detailAcres) / Math.min(listing.acres, detailAcres);
      if (ratio >= ACREAGE_MISMATCH_RATIO) {
        // Overwriting an acreage the card already gave us needs an
        // UNAMBIGUOUS detail-page signal: the parser's own scoped
        // parseDetailPage read, or a price-anchored structured value that
        // cannot belong to a "similar properties nearby" card. Without one,
        // the disagreement is only ever REPORTED — acting on an unscoped
        // page-wide match rewrote a correct 200ac listing to a neighbour's
        // 1.5ac and lost the lead to the 40-acre floor.
        const confirmedAcres = rawDetailAcres !== null
          ? rawDetailAcres
          : parser.extractConfirmedDetailAcres(html, listing.price);
        const canOverride = confirmedAcres != null && confirmedAcres > 0
          && Math.max(listing.acres, confirmedAcres) / Math.min(listing.acres, confirmedAcres) >= ACREAGE_MISMATCH_RATIO;

        parser.recordSourceIssue({
          type: 'markup_drift',
          url: listing.url,
          county: listing.county || null,
          state: listing.state || null,
          page: null,
          error: canOverride
            ? `Card acreage ${listing.acres} disagrees with detail-page structured acreage ${confirmedAcres} by ${ratio.toFixed(1)}x — trusting the detail page ("N-acre lake/park" description mismatch)`
            : `Card acreage ${listing.acres} disagrees with an unconfirmed detail-page acreage ${detailAcres} by ${ratio.toFixed(1)}x — keeping the card value (no price-anchored signal; the detail figure may belong to a nearby-listing card)`,
        });

        if (canOverride) {
          console.warn(`[Scraper] ${parser.name}: acreage mismatch on ${listing.url} — card said ${listing.acres}ac, detail page says ${confirmedAcres}ac; using ${confirmedAcres}ac`);
          listing.acres = confirmedAcres;
        } else {
          console.warn(`[Scraper] ${parser.name}: unconfirmed acreage mismatch on ${listing.url} — card said ${listing.acres}ac, page also shows ${detailAcres}ac; keeping ${listing.acres}ac (reported for review)`);
        }
      }
    }
  }
