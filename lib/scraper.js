    ...
    if (!listing.acres) {
      ...
      if (ratio >= ACREAGE_MISMATCH_RATIO) {
        parser.recordSourceIssue({
          ...
          page: null,
          error: `Card acreage ${listing.acres} disagrees with detail-page structured acreage ${detailAcres} by ${ratio.toFixed(1)}x — trusting the detail page ("N-acre lake/park" description mismatch)`,
        });
        console.warn(`[Scraper] ${parser.name}: acreage mismatch on ${listing.url} — card said ${listing.acres}ac, detail page says ${detailAcres}ac; using ${detailAcres}ac`);
        listing.acres = detailAcres;
      }
    }
  }
