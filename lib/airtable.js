...
const { canonicalCountyName, collapseKey } = require('./county-names');
...
// from its link.
let countyIndex = { byId: new Map(), idByKey: new Map(), idByCollapsedKey: new Map() };
...
function getCountyRecordId(county, state) {
  ...
  const exact = countyIndex.idByKey.get(`${String(county).trim().toLowerCase()}|${stateKey}`);
  if (exact) return exact;
  // Spacing-insensitive fallback: "Le Flore" and "LeFlore" are the same
  // county, and which spelling a listing carries depends on whether it came
  // from the target list or was read off a card. Missing here would silently
  // drop the County link on an otherwise good lead.
  return countyIndex.idByCollapsedKey.get(`${collapseKey(county)}|${stateKey}`) || null;
}
      ...
      base(TABLES.county).select().eachPage(
        (pageRecords, fetchNextPage) => {
          for (const rec of pageRecords) {
            ...
            // Restore the official spelling of a hand-entered county name
            // before anything downstream slugifies it into a URL. Airtable
            // stores "LeFlore"; every land site's URL scheme spells it
            // "le-flore", and the collapsed form 400'd LandWatch's whole
            // Le Flore series nightly. See lib/county-names.js.
            const canonicalCounty = canonicalCountyName(countyName.trim(), stateAb);

            // Index EVERY county row (even without a CPA target) so Land
            // records linking to any county can be resolved to county/state
            countyIndex.byId.set(rec.id, { county: canonicalCounty, state: stateAb });
            countyIndex.idByKey.set(`${canonicalCounty.toLowerCase()}|${stateAb.toUpperCase()}`, rec.id);
            // Keep the raw Airtable spelling resolvable too, so nothing that
            // still carries it loses its County link.
            countyIndex.idByKey.set(`${countyName.trim().toLowerCase()}|${stateAb.toUpperCase()}`, rec.id);
            countyIndex.idByCollapsedKey.set(`${collapseKey(canonicalCounty)}|${stateAb.toUpperCase()}`, rec.id);
            ...
            counties.push({ county: canonicalCounty, state: stateAb, maxCPA, recordId: rec.id });
          }
          fetchNextPage();
        },
        (err) => {
