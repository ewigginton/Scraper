...
function checkDuplicate(listing, dedupIndex) {
  ...
  // Catches the same property listed at slightly different prices on two
  // sites (different $5k fingerprint buckets). Price must agree within 10% —
  // two different tracts of similar size in one county are common, so a
  // location-only match is not enough.
  if (dedupIndex.locationMap && listing.price) {
    const key = locationKey(listing.county, listing.state, listing.acres);
    const candidates = key ? dedupIndex.locationMap.get(key) : null;
    if (candidates) {
      for (const candidate of candidates) {
        const diff = Math.abs(candidate.price - listing.price) / candidate.price;
        if (diff <= LOCATION_PRICE_TOLERANCE) {
          return {
            isDuplicate: true,
            reason: `Location+price match (cross-site duplicate at $${candidate.price.toLocaleString()} vs $${listing.price.toLocaleString()})`,
            matchType: 'location-price',
          };
        }
      }
    }
