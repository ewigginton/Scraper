...
const fs = require('fs');
const { isBlockedHtml, isErrorShellHtml } = require('./block-markers');
...
// Statuses bot walls answer while their challenge runs. A real browser may
// still solve the challenge and render real content, so these route to the
// browser fallback; genuine content statuses (404/410/...) do not.
//
// HTTP 400 is deliberately NOT a bot-wall status. LandWatch answers an
// unsupported URL SHAPE with 400 — production nightlies on Aug 6 and Aug 9
// hit it on /oklahoma-land-for-sale/leflore-county and on an acres-over
// filter segment — while its real bot wall serves 403s and Imperva challenge
// pages. Treating those 400s as blocks fed the blocked-pages circuit breaker
// in lib/parsers/base-parser.js, which abandoned the site's entire remaining
// run over a handful of bad URLs.
function isBotWallStatus(status) {
  return status === 403 || status === 429 || status === 503;
}

// HTTP 400 from these sites means "this URL shape is not supported": the one
// page is dead but the site itself is healthy and every other URL shape in the
// run is unaffected. Callers fail that page (and its pagination series) without
// touching the bot-wall accounting — see lib/parsers/base-parser.js. Retrying
// it through the browser is pointless: the rejection is on the URL, not on the
// client, and the site renders a terminal "LandWatch / 400" error shell.
function isUnsupportedUrlStatus(status) {
  return status === 400;
}
...
async function fetchPageWithBrowser(url, { timeoutMs, challengeWaitMs } = {}) {
  ...
  try {
    ...
    // Poll only genuine challenges — a rendered error shell ("LandWatch /
    // 400") is terminal and will never swap in real content, so waiting the
    // challenge budget on it just burns ~25s per rejected URL.
    while (isBlockedHtml(html) && !isErrorShellHtml(html) && Date.now() < deadline) {
      await page.waitForTimeout(1500);
      html = await page.content();
    }

    // Bot walls answer 403/429/503 while their challenge runs; if the final
    // DOM is no longer a challenge page, the wall solved itself and rendered
    // real content — return it and let the caller's parse decide. Every other
    // 4xx/5xx always throws, so removed-listing detection (404/410) and
    // unsupported-URL detection (400) keep working.
...
module.exports = { isEnabled, reasonUnavailable, fetchPageWithBrowser, warnUnavailableOnce, closeBrowser, isBotWallStatus, isUnsupportedUrlStatus };
