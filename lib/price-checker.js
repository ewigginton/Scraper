    ...
    try {
      ...
      if (dropped) {
        ...
        const noteDetail = target
          ? (promotable
            ? `Now $${Math.round(newCPA).toLocaleString()}/ac vs $${target.toLocaleString()}/ac target — within buy range, moved to New Lead.`
        ...
        if (promotable) {
          // Every lead entering Emma's queue arrives as 'New Lead' — her
          // triage filter is built on that stage. The price-drop story
          // lives in the record's notes and Price Check Log, and the email
          // lists these under PROMOTED TO LEADS.
          updates.Stage = STAGES.newLead;
          report.promoted++;
          report.details.push({
