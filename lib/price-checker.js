    ...
    try {
      ...
      try {
        ...
        if (currentPrice === null) {
          ...
          report.errors++;
          await appendPriceCheckLog(record.id, 'Could not extract price — manual check needed');
          await airtable.updateRecord(record.id, { Stage: 'Manual Check' }, true);
        ...
        if (err.message.includes('404') || err.message.includes('410')) {
          ...
          report.errors++;
          await appendPriceCheckLog(record.id, `Error: ${err.message}`);
          continue;
        }
      }
      ...
      if (listingRemoved) {
        await airtable.updateRecord(record.id, { Stage: 'Not Interested' }, true);
        await appendPriceCheckLog(record.id, 'Listing removed (404)');
        report.removed++;
      ...
      if (createdDate) {
        ...
        if (daysSinceCreated >= settings.priceCheck.expirationDays) {
          await airtable.updateRecord(record.id, { Stage: 'Not Interested' }, true);
          await appendPriceCheckLog(record.id, `Expired after ${Math.round(daysSinceCreated)} days with no actionable price drop`);
          report.expired++;
      ...
      if (currentPrice < storedPrice) {
        ...
        await appendPriceCheckLog(record.id, logEntry);
...
function extractPrice(html) {
  ...
  if (jsonLd) {
    try {
      ...
      if (data.price) return parseFloat(String(data.price).replace(/[^0-9.]/g, ''));
    } catch (_) {}
  }
...
async function appendPriceCheckLog(recordId, message) {
  try {
    // We'd normally read the existing log and append, but for simplicity
    // we'll just update with the timestamped message
