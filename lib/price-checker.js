...
async function runPriceCheck(options = {}) {
  ...
  let watchRecords;
  try {
    watchRecords = await airtable.getRecordsByStage('Watch For Price Drop');
  } catch (err) {
    if (!dryRun) throw err;
    report.errors++;
    report.details.push({
...
/**
 * Mirror a price-cut message into the record's Property Notes so the story
...
async function appendPropertyNote(recordId, message, dryRun = false) {
  try {
    ...
    const existing = record?.fields?.['Property Notes'] || '';
    const updated = mergeNoteEntry(existing, `[${timestamp}] ${message}`);
    await airtable.updateRecord(recordId, { 'Property Notes': updated }, false);
  } catch (err) {
